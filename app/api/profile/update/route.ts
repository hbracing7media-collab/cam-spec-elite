...
export async function POST(req: Request) {
  ...
  if (avatarFile && avatarFile.size > 0) {
    ...
    const filePath = `forum_avatars/${user.id}-${Date.now()}.${ext}`;
  ...
  const { data: profile, error: profileError } = await supabase
    ...
    .upsert({
      id: user.id,
      forum_handle: forum_handle || null,
      forum_avatar_url: forum_avatar_url || null,
  ...
  return NextResponse.json({ ok: true, message: "Profile updated!", forum_avatar_url });
}