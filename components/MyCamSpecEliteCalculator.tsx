...
export default function MyCamSpecEliteCalculator({ shortBlocks, headBuilds, camBuilds }: MyCamSpecEliteProps) {
  ...
  const [camSpecs, setCamSpecs] = useState({
    intDur: 226,
    intLift: 0.585,
    lsa: 114,
  });
  ...
  function handleRunCalc() {
    ...
    try {
      ...
      // Use editable cam specs
      const intDur = camSpecs.intDur || 226;
      const intLift = camSpecs.intLift || 0.585;
      const lsa = camSpecs.lsa || 114;
    ...
    >
      ...
      {selectedBlock && linkedCams.length > 0 && (
        <div style={{ marginBottom: '10px', padding: '8px', borderRadius: '8px', background: 'rgba(2,6,23,0.6)', border: '1px solid rgba(255,43,214,0.2)' }}>
          ...
          {/* Cam Specs Inputs */}
          {selectedCamId && (
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px', fontSize: '10px', marginTop: '6px' }}>
              <div>
                <label style={{ fontSize: '9px', color: '#a5b4fc', display: 'block', marginBottom: '2px' }}>Int Dur @ .050</label>
                <input
                  type="number"
                  value={camSpecs.intDur}
                  onChange={(e) => setCamSpecs({ ...camSpecs, intDur: parseFloat(e.target.value) || 226 })}
                  min="180"
                  max="290"
                  step="1"
                  style={{
              ...
              <div>
                <label style={{ fontSize: '9px', color: '#a5b4fc', display: 'block', marginBottom: '2px' }}>Int Lift (in)</label>
                <input
                  type="number"
                  value={camSpecs.intLift}
                  onChange={(e) => setCamSpecs({ ...camSpecs, intLift: parseFloat(e.target.value) || 0.585 })}
                  min="0.35"
                  max="0.9"
                  step="0.001"
                  style={{
              ...
              <div style={{ gridColumn: '1 / -1' }}>
                <label style={{ fontSize: '9px', color: '#a5b4fc', display: 'block', marginBottom: '2px' }}>LSA (°)</label>
                <input
                  type="number"
                  value={camSpecs.lsa}
                  onChange={(e) => setCamSpecs({ ...camSpecs, lsa: parseFloat(e.target.value) || 114 })}
                  min="104"
                  max="118"
                  step="1"
                  style={{
